...
int main()
{
    ...
    dim3 number_of_blocks((ni + threads_per_block.x - 1) / threads_per_block.x, (nj + threads_per_block.y - 1) / threads_per_block.y);
    ...
    for (int i = 0; i < ni * nj; ++i) {
        if (fabs(temp1[i] - temp1_ref[i]) > maxError) { maxError = fabs(temp1[i] - temp1_ref[i]); }
    }
